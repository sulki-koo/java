...
public class Sound implements Runnable {
	...
	// 배경 음악 재생 메서드
	public void playBgm() {
		playSound(bgmFilePath); // 배경음악을 재생
	}

	// 효과음 1 재생 메서드
	public void playStart() {
		new Thread(() -> playSound(startFilePath)).start(); // 효과음 1을 재생
	}

	// 효과음 2 재생 메서드
	public void playGameOver() {
		new Thread(() -> playSound(gameOverFilePath)).start(); // 효과음 2를 재생
	}

	// 효과음 3 재생 메서드
	public void playClear() {
		new Thread(() -> playSound(clearFilePath)).start(); // 효과음 3을 재생
	}
...
//			System.out.println("효과음 또는 배경음악 재생 중...");
			mp3Player.play(); // 파일 재생
		} catch (Exception e) {
//			System.err.println("재생 오류: " + e.getMessage()); // 오류 처리
